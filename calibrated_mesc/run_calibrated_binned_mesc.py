...
def single_run_of_calibrated_mesc(gwas_variant_ld_scores, gene_ldscores_training1, gene_ldscores_validation1,gene_ldscores_training2, gene_ldscores_validation2, gwas_E_beta_sq, regression_weights, squared_eqtl_effect_threshold, n_reference_snps, n_genetic_genes1, n_genetic_genes2, full_eqtl_dataset_names, eqtl_dataset_names, step1_regression_method, inference_approach, covariance_calibration):
	...
	if inference_approach == '2SLS' or inference_approach == 'binned_2SLS' or inference_approach == 'seperately_binned_2SLS' or inference_approach == 'binned_2SLS_no_train_bin':
...
def print_mesc_regression_results(mesc_obj, output_file):
	...
	t.write('total_h2\t' + str(mesc_obj['total_h2']) + '\t' + 'NA' + '\t' + 'NA' + '\n')

	t.write('nm_h2\t' + str(mesc_obj['nm_h2']) + '\t' + 'NA' + '\t' + 'NA' + '\n')
	t.write('total_med_h2\t' + str(mesc_obj['total_med_h2']) + '\t' + 'NA' + '\t' + 'NA' + '\n')

	for ii, eqtl_cat_med_h2 in enumerate(mesc_obj['med_h2']):
		t.write('category_med_h2_' + str(ii) + '\t' + str(eqtl_cat_med_h2) + '\t' + 'NA' + '\t' + 'NA' + '\n')

	for ii, eqtl_dataset_med_h2 in enumerate(mesc_obj['dataset_med_h2']):
		t.write('dataset_med_h2_' + str(ii) + '\t' + str(eqtl_dataset_med_h2) + '\t' + 'NA' + '\t' + 'NA' + '\n')

	t.write('eqtl_h2\t' + str(mesc_obj['eqtl_h2']) + '\t' + 'NA' + '\t' + str(0.0) + '\n')
...
def load_in_binned_eqtl_ldscores_for_single_training_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, replicate_index, n_bins):
	...
	study_gene_to_study_bin_assignments = {}
	...
	for line in f:
		...
		for line2 in gg:
			...
			study_gene_array.append(study_gene)
		gg.close()
		cis_snp_h2_arr = np.asarray(cis_snp_h2_arr).astype(float)
		study_gene_array = np.asarray(study_gene_array)
		...
		# Create study-gene bin assignments
		edges = np.quantile(cis_snp_h2_arr, np.linspace(0, 1, n_bins + 1))
		bin_idx = np.digitize(cis_snp_h2_arr, edges[1:-1], right=True)


		for ii,study_gene in enumerate(study_gene_array):
			study_gene_to_study_bin_assignments[study_gene] = bin_idx[ii]
	...
	for study_name in study_names:
		for bin_index in range(n_bins):
			study_bin = study_name + ':' + 'bin' + str(bin_index)
			study_bin_names.append(study_bin)
			full_study_names.append(study_name)
	study_bin_names = np.asarray(study_bin_names)
	full_study_names = np.asarray(full_study_names)
	study_bin_to_index = {}
	for ii,study_bin_name in enumerate(study_bin_names):
		study_bin_to_index[study_bin_name] = ii
	...
	#####################################################
	gene_ldscores = np.zeros((len(rsid_to_position), len(study_bin_names)))

	col_index_to_cis_h2s = {}
	for ii in range(len(study_bin_names)):
		col_index_to_cis_h2s[ii] = []

	n_genes = np.zeros(len(study_bin_names))
	avg_cis_h2s = np.zeros(len(study_bin_names))
	...
	for line in f:
		...
		for line2 in gg:
			...
			if standardize_gene_training_scores:
				...
				bin_name = study_gene_to_study_bin_assignments[study_name + ':' + ens_id]
				col_index = study_bin_to_index[study_name + ':bin' + str(bin_name)]
				gene_ldscores[variant_pos, col_index] = gene_ldscores[variant_pos, col_index] + np.square(beta) - np.square(beta_se)
			if ens_id in study_genes:
				continue
			study_genes[ens_id] = 1
			col_index_to_cis_h2s[col_index].append(study_gene_to_cis_snp_h2[study_name + ':' + ens_id])
		gg.close()
	...
	for ii, study_bin_name in enumerate(study_bin_names):
		...
		cis_h2s = np.asarray(col_index_to_cis_h2s[ii])
		n_genes.append(len(cis_h2s))
		avg_cis_h2s.append(np.mean(cis_h2s))
	n_genes = np.asarray(n_genes)
	avg_cis_h2s = np.asarray(avg_cis_h2s)
	


	return gene_ldscores, n_genes, avg_cis_h2s, full_study_names, study_gene_to_study_bin_assignments



def load_in_binned_eqtl_ldscores_for_single_validation_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, replicate_index, n_bins, study_gene_to_study_bin_assignments):
	scale_variant_effects_by_ref_sdev = False
	if training_data_eqtl_ldscores_type == 'MarginalSS':
		scale_variant_effects_by_ref_sdev = True
	...
	return gene_ldscores, n_genes, avg_cis_h2s, full_study_names
...
def load_in_binned_eqtl_ldscores_training(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, n_bins):
	# Do first for replicate 1
	eqtl_ldscores_rep1, n_genes_rep1, avg_cis_h2s_rep1, full_dataset_names, study_gene_to_study_bin_assignments_rep1 = load_in_binned_eqtl_ldscores_for_single_training_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, 0, n_bins)

	# Do for second replicate
	eqtl_ldscores_rep2, n_genes_rep2, avg_cis_h2s_rep2, full_dataset_names, study_gene_to_study_bin_assignments_rep2 = load_in_binned_eqtl_ldscores_for_single_training_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, 1, n_bins)


	return eqtl_ldscores_rep1, eqtl_ldscores_rep2, avg_cis_h2s_rep1, n_genes_rep1, avg_cis_h2s_rep2, n_genes_rep2,study_gene_to_study_bin_assignments_rep1, study_gene_to_study_bin_assignments_rep2, full_dataset_names
...
def load_in_binned_eqtl_ldscores_validation(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, n_bins, study_gene_to_study_bin_assignments_rep1, study_gene_to_study_bin_assignments_rep2):
	# Do first for replicate 1
	eqtl_ldscores_rep1, n_genes_rep1, avg_cis_h2s_rep1, full_dataset_names = load_in_binned_eqtl_ldscores_for_single_validation_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, 1, n_bins, study_gene_to_study_bin_assignments_rep1)

	# Do for second replicate
	eqtl_ldscores_rep2, n_genes_rep2, avg_cis_h2s_rep2, full_dataset_names = load_in_binned_eqtl_ldscores_for_single_validation_replicate(eqtl_training_data_summary_file, training_data_eqtl_ldscores_type, standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, eqtl_cis_snp_h2_summary_file, cis_h2_thresh, 0, n_bins, study_gene_to_study_bin_assignments_rep2)


	return eqtl_ldscores_rep1, eqtl_ldscores_rep2, avg_cis_h2s_rep1, n_genes_rep1, avg_cis_h2s_rep2, n_genes_rep2, full_dataset_names
...
gene_ldscores_training1, gene_ldscores_training2, avg_eqtl_h2s_train_1, n_genes_train_1, avg_eqtl_h2s_train_2, n_genes_train_2, study_gene_to_study_bin_assignments_rep1, study_gene_to_study_bin_assignments_rep2, full_eqtl_dataset_names = load_in_binned_eqtl_ldscores_training(args.eqtl_training_data_summary_file, args.training_data_eqtl_ldscores_type, args.standardize_gene_training_scores, rsid_to_position, rsid_to_variant_stdev, args.eqtl_cis_snp_h2_summary_file, args.cis_h2_thresh, args.n_expr_cis_h2_bins)

# Load in eQTL LD scores for validation data
gene_ldscores_validation1, gene_ldscores_validation2, avg_eqtl_h2s_1, n_genes_1, avg_eqtl_h2s_2, n_genes_2, full_eqtl_dataset_names = load_in_binned_eqtl_ldscores_validation(args.eqtl_validation_data_summary_file, args.validation_data_eqtl_ldscores_type, args.standardize_gene_validation_scores, rsid_to_position, rsid_to_variant_stdev, args.eqtl_cis_snp_h2_summary_file, args.cis_h2_thresh, args.n_expr_cis_h2_bins, study_gene_to_study_bin_assignments_rep1, study_gene_to_study_bin_assignments_rep2)
eqtl_dataset_names = np.sort(np.unique(full_eqtl_dataset_names))

if args.inference_approach == 'seperately_binned_2SLS':
	gene_ldscores_validation2, gene_ldscores_validation1, avg_eqtl_h2s_train_1_tmp, n_genes_train_1_tmp, avg_eqtl_h2s_train_2_tmp, n_genes_train_2_tmp, study_gene_to_study_bin_assignments_rep1_tmp, study_gene_to_study_bin_assignments_rep2_tmp, full_eqtl_dataset_names_tmp = load_in_binned_eqtl_ldscores_training(args.eqtl_validation_data_summary_file, args.validation_data_eqtl_ldscores_type, args.standardize_gene_validation_scores, rsid_to_position, rsid_to_variant_stdev, args.eqtl_cis_snp_h2_summary_file, args.cis_h2_thresh, args.n_expr_cis_h2_bins)
