...
for tissue_iter in range(n_tissues):
	...
	tissue_sumstat_output_file_full = simulated_learned_gene_models_dir + simulation_name_string + '_tissue' + str(tissue_iter) + '_' + str(eqtl_sample_size) + '_full_eqtl_sumstats.txt'

	t.write('tissue' + str(tissue_iter) + '\t' + str(int(eqtl_sample_size/2)) + '\t' + str(int(eqtl_sample_size/2)) + '\t' + str(eqtl_sample_size) + '\t' + tissue_sumstat_output_file1 + '\t' + tissue_sumstat_output_file2 + '\t' + tissue_sumstat_output_file_full + '\n')
t.close()
