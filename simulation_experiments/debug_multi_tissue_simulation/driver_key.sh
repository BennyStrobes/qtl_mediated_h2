...
# Output directories
############################
output_root_dir="/n/scratch/users/b/bes710/qtl_mediated_h2/simulation_experiments/debug_multi_tissue_simulation/"
perm_output_root_dir="/n/groups/price/ben/qtl_mediated_h2/simulation_experiments/debug_multi_tissue_simulation/"
...
for simulation_number in $(seq 1 200); do 
	simulation_name_string="simulation_"${simulation_number}"_chrom"${chrom_string}"_cis_window_"${cis_window}"_ss_"${n_gwas_individuals}"_ge_h2_"${ge_h2}"_qtl_arch_"${eqtl_architecture}"_n_tiss_"${n_tissues}
	sh extract_true_gene_ld_scores.sh $simulation_number $chrom_string $simulation_name_string $simulation_genotype_dir $simulated_gene_expression_dir $calibrated_mesc_code_dir $estimated_cis_snp_h2_dir
done
fi
