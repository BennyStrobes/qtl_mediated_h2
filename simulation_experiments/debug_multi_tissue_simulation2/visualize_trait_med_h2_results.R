...
plot_mediated_h2_inf_ss <- function(method_names, run_strings, organized_trait_med_h2_results_dir) {
	method_names_arr <- c()
	med_h2_arr <- c()
	med_h2_lb_arr <- c()
	med_h2_ub_arr <- c()
	for (method_iter in 1:length(method_names)) {
		method_name = method_names[method_iter]
		run_string = run_strings[method_iter]


		filer = paste0(organized_trait_med_h2_results_dir, "med_h2_5_causal_tissue_", run_string, "_sim_results_calibrated_ldsc_summary_averaged.txt")
		tmp_df <- read.table(filer, header=TRUE, sep="\t")

		index = tmp_df[(tmp_df$method=="calibrated_mesc" & tmp_df$heritability_type=="causal_tissue_med_h2"),]

		method_names_arr <- c(method_names_arr, method_name)
		med_h2_arr <- c(med_h2_arr, index$est_h2[1])
		med_h2_lb_arr <- c(med_h2_lb_arr, index$est_h2_lb[1])
		med_h2_ub_arr <- c(med_h2_ub_arr, index$est_h2_ub[1])
		sim_h2 = index$sim_h2[1]
	}

	df2 <- data.frame(method=as.character(method_names_arr), est_h2=med_h2_arr, est_h2_lb=med_h2_lb_arr, est_h2_ub=med_h2_ub_arr)
	df2$method = factor(df2$method,levels=method_names_arr)


	pp<-ggplot(data=df2, aes(x=method, y=est_h2)) +
  		geom_bar(stat="identity", position=position_dodge()) +
  		geom_errorbar(aes(ymin=est_h2_lb, ymax=est_h2_ub), width=.4, position=position_dodge(.9))  +
  		#scale_fill_manual(values=c("skyblue", "grey", "grey"))+
  		figure_theme() +
  		labs(x="Method", y="Mediated h2", fill="", title=paste0("Causal tissue")) +
  		geom_hline(yintercept=sim_h2, linetype=2) 
...
method_names <- c()
for (bin_iter in 1:7) {
	binned_run_string = paste0(non_med_anno, "_", simulated_gt_architecture, "_linear_", sq_sumstat_threshold, "_true_binned_2SLS_", bin_iter, "_INF_SS_weighted")
	run_strings <- c(run_strings, binned_run_string)
	method_names <- c(method_names, paste0("bins_", bin_iter))
}
run_string = paste0(non_med_anno, "_", simulated_gt_architecture, "_",inference_gt_architecture,"_", sq_sumstat_threshold, "_2SLS_INF_SS_weighted")
run_strings <- c(run_strings, run_string)
method_names <- c(method_names, "inv_var")



joint<-plot_mediated_h2_inf_ss(method_names, run_strings, organized_trait_med_h2_results_dir)

# Save
output_file <- paste0(visualize_trait_med_h2_dir, run_string, "_calibrated_mesc_tissue_simulation_causal_tissue_stdExpr_med_vs_non_causal_tissue_med_se_barplot_INF.pdf")
ggsave(joint, file=output_file, width=7.2, height=4.5, units="in")
print(output_file)
