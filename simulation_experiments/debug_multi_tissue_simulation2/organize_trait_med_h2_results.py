...
def print_temp_line2(method, eqtl_sample_size, t, sim_total_h2, est_total_h2, heritability_type):
	sim_mean = np.mean(sim_total_h2)
	est_mean = np.mean(est_total_h2)

	bias = sim_total_h2 - est_total_h2

	est_bias_lb = np.mean(bias) - 1.96*(np.std(bias)/np.sqrt(len(bias)))
	est_bias_ub = np.mean(bias) + 1.96*(np.std(bias)/np.sqrt(len(bias)))


	est_mean_lb = est_bias_lb
	est_mean_ub =  est_bias_ub
...
def average_results_across_simulations_5_causal_tissue(sim_nums, eqtl_sample_sizes, trait_med_h2_inference_dir, avg_results_summary_file, methods, clean_method_names, invalid_sims, run_string, weighting, sim_heritabilities, permuted_eqtls=False, variance_weighting=False, weighted=False):
	t = open(avg_results_summary_file,'w')
	t.write('method\teqtl_sample_size\theritability_type\tsim_h2\test_h2\test_h2_lb\test_h2_ub\n')
	for ii,method in enumerate(methods):
		for eqtl_sample_size in eqtl_sample_sizes:
			...
			for sim_num in sim_nums:
				if weighted:
					...
					filer = trait_med_h2_inference_dir + 'simulation_' + str(sim_num) + '_chrom1_2_cis_window_500000_ss_100000_ge_h2_05_qtl_arch_default_n_tiss_5_' + run_string + '_' + str(eqtl_sample_size) + '_' + method + '_jk_results.txt'
				if sim_num in invalid_sims:
					continue
				if os.path.isfile(filer) == False:
					print(sim_num)
...
avg_results_summary_file = visualize_trait_med_h2_dir+ 'med_h2_5_causal_tissue_' + run_string + '_INF_SS_unweighted_sim_results_calibrated_ldsc_summary_averaged.txt'
clean_method_names = ['uncalibrated_mesc', 'calibrated_mesc']
methods = ['uncalibrated_mesc', 'calibrated_mesc']
average_results_across_simulations_5_causal_tissue(sim_nums, eqtl_sample_sizes, trait_med_h2_inference_dir, avg_results_summary_file, methods, clean_method_names, invalid_sims, run_string, weighting, sim_heritabilities, variance_weighting=False, weighted=False)
print(avg_results_summary_file)
