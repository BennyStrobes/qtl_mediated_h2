...
class Bayesian_LMM_SS_h2_med_inference(object):
	def __init__(self, gwas_beta, variant_ld_scores, N_gwas, n_gwas_snps, eqtl_beta, eqtl_ldscore, eqtl_ldscore_ratios, eqtl_position, eqtl_sample_size, n_eqtl_snps):
		# Load in gwas data
		...
		self.eqtl_ldscore_ratio = eqtl_ldscore_ratios
	...
	def fit(self, total_iterations=15000, burn_in_iterations=10000, update_gwas_resid_var=True, update_eqtl_resid_var=True, v0=2.0, s_sq=0.0, cc=0.0):
		...
		for itera in range(total_iterations):
			...
			if itera > burn_in_iterations:
				...
				alt_med_h2, total_h2, alt_nm_h2 = self.compute_overlapping_med_h2()
	...
	def update_deltas(self):
		...
		for gg in np.random.permutation(range(self.GG)):
			...
			posterior_vars = 1.0/((np.square(self.alpha[gg])/(self.gwas_resid_var*cis_gwas_beta_var)) + (self.eqtl_ldscore_ratio[gg]/(self.eqtl_beta_var*self.eqtl_resid_vars[gg])) + (1.0/marginal_delta_vars))
			posterior_means = ((cis_gwas_beta*self.alpha[gg]/(self.gwas_resid_var*cis_gwas_beta_var)) + (self.eqtl_beta[gg]*np.sqrt(self.eqtl_ldscore_ratio[gg])/(self.eqtl_beta_var*self.eqtl_resid_vars[gg])))*posterior_vars
