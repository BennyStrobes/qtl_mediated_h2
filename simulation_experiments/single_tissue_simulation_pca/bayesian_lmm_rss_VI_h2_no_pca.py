...
class Bayesian_LMM_RSS_h2_inference(object):
	...
	def fit(self, total_iterations=15000, update_resid_var_bool=False, univariate_updates=True,cc=1e-6):
		""" Fit the model.
		"""
		...
		for itera in range(total_iterations):
			...
			self.h2_independent = self.gamma_var*self.KK
			self.h2_full = self.get_ld_dependent_h2s()
	...
	def update_gamma_var(self, v0=0.0, s_sq=0.0, cc=1e-6):
		...
		all_gammas = []
		for window_name in self.windows:
			window_gammas = np.square(self.gamma[window_name]) + np.diag(self.gamma_mu_var[window_name])
	...
	def update_window_genetic_var(self, window_name, LD_mat):
		KK = len(self.gamma[window_name])
		e_gamma_t_gamma = self.gamma_mu_var[window_name] + np.dot(self.gamma[window_name].reshape(KK, 1), self.gamma[window_name].reshape(1, KK))
		self.window_nm_h2[window_name] = np.trace(np.dot(LD_mat, e_gamma_t_gamma))
		return
	...
	def univariate_update_gamma_in_single_window(self, window_name, LD):
		...
		window_gamma_mu_var = np.copy(np.diag(self.gamma_mu_var[window_name]))
		...
		self.gamma_mu_var[window_name] = np.diag(window_gamma_mu_var)
	...
	def multivariate_update_gamma_in_single_window(self, window_name, LD):
		...
		self.gamma[window_name] = posterior_mean
		self.gamma_mu_var[window_name] = cov
	...
	def initialize_variables(self):
		...
		for window_name in self.windows:
			...
			self.gamma_mu_var[window_name] = np.eye(self.window_info[window_name]['n_snps'])
