...
class Bayesian_LMM_RSS_med_h2_inference(object):
	def __init__(self, window_info, gene_info, N_gwas, N_eqtl, tmp_output_file):
		...
		self.tt.write('Iteration\tnm_h2\talt_nm_h2\tmed_h2\talt_med_h2\talt2_nm_h2\teqtl_h2\talt_eqtl_h2\ttotal_h2\n')
		
		return



	def fit(self, total_iterations=15000, burn_in_iterations=10000, update_resid_var_bool=False, delta_updates='univariate',cc=1e-6):
		""" Fit the model.
		"""
		...
		for itera in range(total_iterations):
			...
			# Update gamma_var
			self.update_gamma_var(cc=cc)

			# Update delta var
			self.update_delta_vars(cc=cc)

			# Update alpha var
			self.update_alpha_var(cc=cc)
			...
			if itera > burn_in_iterations:
				...
				nm_h2_ld_depen, med_h2_ld_depen, total_h2_ld_depen = self.get_ld_dependent_h2s()
				...
				print('total: ' + str(total_h2_ld_depen))
				print(np.sort(alt_eqtl_h2s))


				self.tt.write(str(itera) + '\t' + str(nm_h2) + '\t' + str(nm_h2_ld_depen) + '\t' + str(med_h2) + '\t' + str(med_h2_alt) + '\t' + str(med_h2_ld_depen) + '\t' + str(avg_eqtl_h2) + '\t' + str(np.mean(alt_eqtl_h2s)) + '\t' + str(total_h2_ld_depen) + '\n')
				self.tt.flush()
	...
	def get_ld_dependent_h2s(self):
		...
		for window in self.windows:
			...
			total_h2 = total_h2 + self.window_total_h2[window]
		return nm, med, total_h2
	...
	def update_window_genetic_var(self, window_name, LD_mat):
		...
		self.window_total_h2[window_name] = np.dot(np.dot(window_nm_causal_effects + window_med_causal_effects, LD_mat), window_nm_causal_effects + window_med_causal_effects)
	...
	def update_gamma_and_alpha_in_single_window(self, window_name, LD):
	...
	def alpha_update_for_single_gene(self, window_gwas_beta_resid, LD, gene_name, window_gwas_resid_var):
		...
		genetic_gene_var = np.dot(np.dot(delta, gene_ld), delta)
	...
	def initialize_variables(self):
		...
		for window_name in self.windows:
			...
			self.window_total_h2[window_name] = 0.0
