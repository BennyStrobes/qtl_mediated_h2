...
class Bayesian_LMM_RSS_med_h2_inference(object):
	...
	def update_delta_vars(self, v0=0.0, s_sq=0.0, cc=1e-6):
		...
		for gene in self.genes:
			...
			self.delta_vars[gene] = np.sum(np.square(delta))/len(delta)
	...
	def initialize_variables(self):
		...
		for gene in self.genes:
			...
			gene_mod.fit(burn_in_iterations=4000, total_iterations=6000)

			print(str(np.mean(gene_mod.sampled_h2)) + '\t' + str(np.sum(np.square(self.gene_info[gene]['sim_delta']))))
			print(np.corrcoef(np.mean(gene_mod.sampled_gammas,axis=0), self.gene_info[gene]['sim_delta']))
			aa.append(np.mean(gene_mod.sampled_h2))
			bb.append(np.sum(np.square(self.gene_info[gene]['sim_delta'])))
			# Initialize
			self.deltas[gene] = self.gene_info[gene]['sim_delta']
