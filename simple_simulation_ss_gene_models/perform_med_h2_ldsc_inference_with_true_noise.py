...
def mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq_v3(gwas_z_scores, N_gwas, LD, causal_eqtl_effects, causal_eqtl_indices, est_gene_ld_scores,fit_intercept_bool=False):
	# Get number of genes
	n_genes = len(causal_eqtl_indices)
	# Get number of snps
	n_snps = len(gwas_z_scores)
...
def mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq_v4(gwas_z_scores, N_gwas, LD, causal_eqtl_effects, causal_eqtl_indices, est_gene_ld_scores,fit_intercept_bool=False):
	# Get number of genes
	n_genes = len(causal_eqtl_indices)
	# Get number of snps
	n_snps = len(gwas_z_scores)
	...
	VV[1,1] = np.var(true_gene_ld_scores,ddof=1)
	...
	return med_h2_adj, np.cov(np.transpose(true_joint_ld_scores))[0,1], np.cov(np.transpose(joint_ld_scores))[0,1]
...
def extract_gene_ld_scores(simulated_gene_models_dir, eqtl_ss, simulation_name_string):
	...
	return np.sum(gene_ld_scores,axis=0)
...
est_gene_ld_scores = extract_gene_ld_scores(simulated_gene_models_dir, eqtl_ss, simulation_name_string)


# Print results to output
output_file = mediated_h2_results_dir + simulation_name_string + selection + '_selection_with_known_noise_eqtl_ss_' + str(eqtl_ss)+ 'med_h2_ldsc_style_summary.txt'
t = open(output_file,'w')
# Header
t.write('sim_h2\tsim_nm_h2\tsim_med_h2')
t.write('\tmed_sldsc_est_noisy_gene' + '\t' + 'med_sldsc_est_noisy_gene_adjusted' + '\t' + 'med_sldsc_est_w_intercept_noisy_gene\tmed_sldsc_est_w_intercept_noisy_gene_adj\tmed_sldsc_est_noisy_gene_adjusted_v2\tmed_sldsc_est_w_intercept_noisy_gene_adj_v2\tmed_sldsc_est_w_intercept_noisy_gene_adj_v3\tmed_sldsc_est_w_intercept_noisy_gene_adj_v4\ttrue_cov\test_cov')
t.write('\n')
...
med_sldsc_est_w_intercept_noisy_gene_adj_v3 = mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq_v3(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices, est_gene_ld_scores, fit_intercept_bool=True)
med_sldsc_est_w_intercept_noisy_gene_adj_v4, true_cov, est_cov = mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq_v4(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices, est_gene_ld_scores, fit_intercept_bool=True)

t.write('\t' + str(med_sldsc_est_noisy_gene) + '\t' + str(med_sldsc_est_noisy_gene_adj))
t.write('\t' + str(med_sldsc_est_w_intercept_noisy_gene) + '\t' + str(med_sldsc_est_w_intercept_noisy_gene_adj))
t.write('\t' + str(med_sldsc_est_noisy_gene_adj_v2) + '\t' + str(med_sldsc_est_w_intercept_noisy_gene_adj_v2))
t.write('\t' + str(med_sldsc_est_w_intercept_noisy_gene_adj_v3) + '\t' + str(med_sldsc_est_w_intercept_noisy_gene_adj_v4))
t.write('\t' + str(true_cov) + '\t' + str(est_cov))
