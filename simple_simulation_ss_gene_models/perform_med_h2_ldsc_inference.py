...
def run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, gene_ld_scores, N_gwas, n_snps, n_genes, average_ge_h2, fit_intercept=True, noise_vec=np.zeros(2)):
	if fit_intercept:
		# Prepare LDSC data
...
noisy_gene_ld_score_file = simulated_learned_gene_models_dir + simulation_name_string + 'estimated_noisy_gene_related_snp_annotations_' + eqtl_ss + '.npy'
noisy_gene_ld_scores = np.load(noisy_gene_ld_score_file)
# Extract estimated noise on above estimated gene ld scores
noisy_gene_ld_score_noise_file = simulated_learned_gene_models_dir + simulation_name_string + 'estimated_noisy_gene_related_snp_annotation_noises_' + eqtl_ss + '.npy'
...
posterior_t_h2_t_residvar_gene_ld_score_file = simulated_learned_gene_models_dir + simulation_name_string + 'posterior_known_h2_known_resid_var_gene_related_snp_annotations_' + eqtl_ss + '.npy'
...
posterior_blup_gene_ld_score_file = simulated_learned_gene_models_dir + simulation_name_string + 'posterior_blup_gene_related_snp_annotations_' + eqtl_ss + '.npy'
posterior_blup_gene_ld_scores = np.load(posterior_blup_gene_ld_score_file)

'''
# Extract posterior estimated gene ld scores (known gene h2, unknown residual variance)
posterior_t_h2_f_residvar_gene_ld_score_file = simulated_learned_gene_models_dir + simulation_name_string + 'posterior_known_h2_unknown_resid_var_gene_related_snp_annotations_' + eqtl_ss + '.npy'
posterior_t_h2_f_residvar_gene_ld_scores = np.load(posterior_t_h2_f_residvar_gene_ld_score_file)
# Extract posterior estimated gene ld scores (unknown gene h2, unknown residual variance)
posterior_f_h2_f_residvar_gene_ld_score_file = simulated_learned_gene_models_dir + simulation_name_string + 'posterior_unknown_h2_unknown_resid_var_gene_related_snp_annotations_' + eqtl_ss + '.npy'
posterior_f_h2_f_residvar_gene_ld_scores = np.load(posterior_f_h2_f_residvar_gene_ld_score_file)
# Extract expected gene h2s from posterior model
posterior_gene_h2_est_file = simulated_learned_gene_models_dir + simulation_name_string + 'est_gene_h2_unknown_h2_unknown_resid_var_' + str(eqtl_ss) + '.npy'
posterior_gene_h2_ests = np.load(posterior_gene_h2_est_file)
'''
# Get simulated noise
sim_noise = np.var(noisy_gene_ld_scores,ddof=1) - np.var(sim_gene_ld_scores,ddof=1)
# Get estimated gene cis-h2
blup_cis_gene_h2_file = simulated_learned_gene_models_dir + simulation_name_string + 'est_gene_h2_blup_' + str(eqtl_ss) + '.npy'
blup_cis_gene_h2 = np.load(blup_cis_gene_h2_file)
...
#t.write('\tmed_h2_est_posterior_gene_ld_scores_known_gene_h2_unknown_residvar')
#t.write('\tmed_h2_est_posterior_gene_ld_scores_unknown_gene_h2_unknown_residvar')
#t.write('\tmed_h2_est_posterior_gene_ld_scores_unknown_gene_h2_unknown_residvar_scale_by_est_gene_h2')
t.write('\n')
# First couple of simulated elements
t.write(str(sim_genetic_var) + '\t' + str(sim_nm_genetic_var) + '\t' + str(sim_med_genetic_var))
# Print next two noise-relevent elements
t.write('\t' + str(sim_noise) + '\t' + str(noise_gene_ld_score_noise_est))
# Print est cis gene h2
t.write('\t' + str(np.mean(blup_cis_gene_h2)))

###########################
# Run S-LDSC to get mediated heritabilities
###########################
sim_average_ge_h2 = .05


# true, simulated gene ld scores
med_h2_sim_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, sim_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_sim_gene_ld_scores))

# estimated, noisy gene ld scores
med_h2_est_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, noisy_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_est_gene_ld_scores))

# estimated, noisy gene ld scores with dissattenuation correction based on true sim variance
noise_vec = np.zeros(2)
noise_vec[1] = sim_noise
med_h2_est_gene_ld_scores_dissatten_true_noise = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, noisy_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2, noise_vec=noise_vec)
t.write('\t' + str(med_h2_est_gene_ld_scores_dissatten_true_noise))

# estimated, noisy gene ld scores with dissattenuation correction based on estimated variance
noise_vec = np.zeros(2)
noise_vec[1] = noise_gene_ld_score_noise_est
med_h2_est_gene_ld_scores_dissatten_est_noise = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, noisy_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2, noise_vec=noise_vec)
t.write('\t' + str(med_h2_est_gene_ld_scores_dissatten_est_noise))

# posterior gene ld scores (known gene h2, known resid var)
med_h2_posterior_t_h2_t_residvar_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, posterior_t_h2_t_residvar_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_posterior_t_h2_t_residvar_gene_ld_scores))

# posterior gene ld scores (BLUP)
med_h2_posterior_blup_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, posterior_blup_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_posterior_blup_gene_ld_scores))


'''
# posterior gene ld scores (known gene h2, unknown resid var)
med_h2_posterior_t_h2_f_residvar_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, posterior_t_h2_f_residvar_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_posterior_t_h2_f_residvar_gene_ld_scores))

# posterior gene ld scores (unknown gene h2, unknown resid var)
med_h2_posterior_f_h2_f_residvar_gene_ld_scores = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, posterior_f_h2_f_residvar_gene_ld_scores, N_gwas, n_snps, n_genes, sim_average_ge_h2)
t.write('\t' + str(med_h2_posterior_f_h2_f_residvar_gene_ld_scores))

# posterior gene ld scores (unknown gene h2, unknown resid var) scaled by est gene h2
med_h2_posterior_f_h2_f_residvar_gene_ld_scores_scaled = run_sldsc_to_get_mediated_heritabilities(gwas_chi_sq, var_ld_scores, posterior_f_h2_f_residvar_gene_ld_scores, N_gwas, n_snps, n_genes, np.mean(posterior_gene_h2_ests))
t.write('\t' + str(med_h2_posterior_f_h2_f_residvar_gene_ld_scores_scaled))
'''
