...
def simulate_data(gwas_ss, n_snps, eqtl_ss_1, eqtl_ss_2, eqtl_ss_3, med_h2_1, med_h2_2, med_h2_3, ge_h2_1, ge_h2_2, ge_h2_3, corr):
...
def scipy_odr_mediated_effect_est(gwas_beta, gene_betas, eqtl_sss, gwas_ss, sstol_string='default'):
	...
	if sstol_string == 'default':
		odr_obj = odr.ODR(data, linear, maxit=20000, beta0=[0.0, 0.0, 0.0])
	elif sstol_string == 'stringent':
		odr_obj = odr.ODR(data, linear, sstol=1e-19, maxit=20000, beta0=[0.0, 0.0, 0.0])
	else:
...
def scipy_odr_mediated_effect_est_per_snp_var(gwas_beta, gene_betas, gene_beta_variances, gwas_beta_variances, sstol_string='default'):
	...
	linear = odr.Model(linfit)
	data = odr.Data(gene_betas, gwas_beta, wd=eqtl_weights,we=gwas_weights)
	if sstol_string == 'default':
		odr_obj = odr.ODR(data, linear, maxit=20000, beta0=[0.0,0.0, 0.0])
	elif sstol_string == 'stringent':
		odr_obj = odr.ODR(data, linear,sstol=1e-19, maxit=20000, beta0=[0.0,0.0, 0.0])
...
def compute_odr_mediated_effect_loss_np(est_alphas, est_delta, gwas_beta, eqtl_beta,eqtl_sss, gwas_ss):
	loss1 = np.square(gwas_beta - np.matmul(eqtl_beta + est_delta, est_alphas))*gwas_ss
	loss2 = np.matmul(np.square(est_delta), eqtl_sss)
...
def compute_odr_mediated_effect_loss_per_snp_var_np(est_alphas, est_delta, gwas_beta, eqtl_beta,eqtl_beta_variances, gwas_beta_variances):
	loss1 = np.square(gwas_beta - np.matmul(eqtl_beta + est_delta, est_alphas))/gwas_beta_variances
	loss2 = np.sum((np.square(est_delta)*np.transpose(1.0/eqtl_beta_variances)),axis=1)
...
t.write('method\tsim\tsim_alpha_1\test_alpha_1\tsim_alpha_2\test_alpha_2\tsim_alpha_3\test_alpha_3\tloss\n')
...
for sim_iter in range(n_sims):
	...
	# First simulate data
	sim_alpha_1, sim_alpha_2, sim_alpha_3, Y, E_1, E_2, E_3, Y_geno, E_1_geno, E_2_geno, E_3_geno, gene_causal_eqtl_effects = simulate_data(gwas_ss, n_snps, eqtl_ss_1, eqtl_ss_2, eqtl_ss_3, med_h2_1, med_h2_2, med_h2_3, ge_h2_1, ge_h2_2, ge_h2_3, sim_corr)
	...
	eqtl_2_beta, eqtl_2_beta_se = get_marginal_summary_statistics(E_2, E_2_geno)
	eqtl_3_beta, eqtl_3_beta_se = get_marginal_summary_statistics(E_3, E_3_geno)


	# Organize data
	gene_betas = np.row_stack((eqtl_1_beta, eqtl_2_beta, eqtl_3_beta))
	eqtl_sss = np.asarray([eqtl_ss_1, eqtl_ss_2, eqtl_ss_3])
	gene_beta_variances = np.row_stack((np.square(eqtl_1_beta_se), np.square(eqtl_2_beta_se), np.square(eqtl_3_beta_se)))
	...
	olser = sm.OLS(gwas_beta, gene_causal_eqtl_effects).fit()
	t.write(method + '\t' + str(sim_iter) + '\t' + str(sim_alpha_1) + '\t' + str(olser.params[0]) + '\t' + str(sim_alpha_2) + '\t' + str(olser.params[1]) + '\t' + str(sim_alpha_3) + '\t' + str(olser.params[2]) + '\t' + 'NA' + '\n')
	...
	# Optimize fxn
	est_alphas, est_delta = scipy_odr_mediated_effect_est(gwas_beta, gene_betas, eqtl_sss, gwas_ss, sstol_string='default')
	...
	# Print to output
	t.write(method + '\t' + str(sim_iter) + '\t' + str(sim_alpha_1) + '\t' + str(est_alphas[0]) + '\t' + str(sim_alpha_2) + '\t' + str(est_alphas[1]) + '\t' + str(sim_alpha_3) + '\t' + str(est_alphas[2]) + '\t' + str(loss) + '\n')

	#################################
	# Estimate alphas using scipy ODR
	method = 'scipy_odr_effect_est_stringent'
	# Optimize fxn
	est_alphas, est_delta = scipy_odr_mediated_effect_est(gwas_beta, gene_betas, eqtl_sss, gwas_ss, sstol_string='stringent')
	# Compute loss given optimized parameters
	loss = compute_odr_mediated_effect_loss_np(est_alphas, est_delta, gwas_beta, np.transpose(gene_betas),eqtl_sss, gwas_ss)
	# Print to output
	t.write(method + '\t' + str(sim_iter) + '\t' + str(sim_alpha_1) + '\t' + str(est_alphas[0]) + '\t' + str(sim_alpha_2) + '\t' + str(est_alphas[1]) + '\t' + str(sim_alpha_3) + '\t' + str(est_alphas[2]) + '\t' + str(loss) + '\n')
