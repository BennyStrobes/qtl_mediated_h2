...
class Bayesian_LMM_RSS_med_h2_inference(object):
	...
	def fit(self, total_iterations=15000, burn_in_iterations=10000, update_resid_var_bool=False):
		...
		for itera in range(total_iterations):
			...
			if itera > burn_in_iterations:
				...
				self.tt.write(str(itera) + '\t' + str(nm_h2) + '\t' + str(med_h2) + '\t' + str(med_h2_alt) + '\t' + str(avg_eqtl_h2) + '\n')
				self.tt.flush()
	...
	def update_gamma_delta_and_alpha_in_single_window(self, window_name):
		...
		self.update_gamma_and_alpha_in_single_window(window_name, self.window_info[window_name]['LD'])
		# Update delta in single window
		self.update_delta_in_single_window(window_name, self.window_info[window_name]['LD'], self.window_info[window_name]['LD'])
	...
	def initialize_variables(self):
		...
		for gene in self.genes:
			...
			gene_LD = self.window_info[gene_window]['LD'][:, gene_cis_snp_indices][gene_cis_snp_indices, :]
			...
			# Initialize
			self.deltas[gene] = np.copy(gene_beta)
