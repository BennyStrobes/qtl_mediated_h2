...
def get_gwas_variant_ld_scores(gwas_beta, gwas_rsids, quasi_ld_window_summary_file):
	...
	return np.hstack(var_ld_score), window_to_ld_files, ld_mat
...
def load_in_eqtl_data(eqtl_sumstat_file, snp_name_to_position, ld_mat, eqtl_sample_size, eqtl_ld='out_of_sample'):
	...
	for gene_name in gene_names:
		...
		gene_cis_snp_positions = gene_snp_positions[gene_cis_snp_indices==1]
		
		eqtl_ld_scores = np.sum(squared_adj_ld_mat[gene_cis_snp_positions,:], axis=0)



		# Add to global array
		gene_arr.append(gene_name)
		beta_arr.append(eqtl_gene_beta[gene_cis_snp_indices==1])
		beta_se_arr.append(eqtl_gene_beta_se[gene_cis_snp_indices==1])
...
def sumstat_proportional_joint_reml_loss(gwas_beta, gwas_beta_var, gwas_ld_scores, eqtl_beta, eqtl_beta_var, eqtl_ld_scores,eqtl_mask, alpha_sq_variable, eqtl_beta_sq_variable, beta_sq_variable, n_snps, per_snp_eqtl_h2_est_lb, per_snp_gwas_h2_est_lb, gwas_noise, eqtl_noise):
	...
	pred_per_snp_gwas_h2 = (gwas_ld_scores*beta_sq_variable) + alpha_sq_variable*(tf.math.reduce_sum((big_eqtl_beta_sq_variable),axis=0))
	...
	eqtl_log_like = compute_univariate_gaussian_log_like(eqtl_beta[eqtl_indices], 0.0, eqtl_beta_var[eqtl_indices] + eqtl_noise + big_eqtl_beta_sq_variable[eqtl_indices])
...
def med_h2_with_sumstat_reml(gwas_beta, gwas_beta_se, gwas_ld_scores, eqtl_beta, eqtl_beta_se, eqtl_ld_scores, eqtl_n_cis_snps, max_epochs=40000, conv_thresh=1e-12, intercept_variables=False):
	...
	beta_sq_variable = tf.Variable(initial_value=0.00001,trainable=True, name='beta_sq')
	alpha_sq_variable = tf.Variable(initial_value=0.00001,trainable=True, name='alpha_sq')
	eqtl_beta_sq_variable = tf.Variable(initial_value=np.ones(n_genes)*0.001 - per_snp_eqtl_h2_est_lb,trainable=True, name='eqtl_beta_sq', dtype=tf.float32)

	gwas_noise = tf.Variable(initial_value=0.0000000,trainable=True, name='gwas_noise')
	eqtl_noise = tf.Variable(initial_value=0.0000000,trainable=True, name='eqtl_noise')
...
gwas_variant_ld_scores, window_to_ld_files, ld_mat = get_gwas_variant_ld_scores(gwas_beta, gwas_rsids, quasi_ld_window_summary_file)
...
genes, eqtl_beta, eqtl_beta_se, eqtl_ldscore, eqtl_position, eqtl_cis_snp_position, eqtl_n_cis_snps = load_in_eqtl_data(eqtl_sumstat_file, snp_name_to_position, ld_mat ,eqtl_sample_size, eqtl_ld='out_of_sample')




# Generate matrix form of eqtl data
eqtl_beta_mat = get_matrix_form_of_eqtl_data(eqtl_beta, eqtl_cis_snp_position, 0.0, len(gwas_variant_ld_scores))
eqtl_beta_se_mat = get_matrix_form_of_eqtl_data(eqtl_beta_se, eqtl_cis_snp_position, 1.0/np.sqrt(eqtl_sample_size), len(gwas_variant_ld_scores))
eqtl_ld_score_mat = np.asarray(eqtl_ldscore)


output_file = trait_med_h2_inference_dir + simulation_name_string+ '_' + str(eqtl_sample_size) + '_joint_reml2_w_intercepts.txt'
...
est_med_h2, est_nm_h2, est_gene_cis_h2 = med_h2_with_sumstat_reml(gwas_beta, gwas_beta_se, gwas_variant_ld_scores, eqtl_beta_mat, eqtl_beta_se_mat, eqtl_ld_score_mat, eqtl_n_cis_snps, max_epochs=30000, intercept_variables=True)
