...
import bayesian_lmm_rss_med_h2_known_delta
...
def load_in_eqtl_data(eqtl_sumstat_file, window_info, window_names, simulated_gene_expression_dir, simulation_name_string):
	...
	for gene_name in gene_names:
		...
		causal_eqtl_effect_file = simulated_gene_expression_dir + simulation_name_string + '_' + gene_name + '_causal_eqtl_effects.npy'
		sim_delta = np.load(causal_eqtl_effect_file)

		if len(sim_delta) != np.sum(gene_cis_snp_indices):
		...
		gene_info[gene_name]['sim_delta'] = sim_delta
...
if resid_var_bool == 'True':
	...
	mod = bayesian_lmm_rss_med_h2_known_delta.Bayesian_LMM_RSS_med_h2_inference(window_info, gene_info, N_gwas, N_eqtl, tmp_output_file)
