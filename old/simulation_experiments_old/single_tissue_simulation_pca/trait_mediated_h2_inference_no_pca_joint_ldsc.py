...
def sumstat_proportional_joint_reml_loss(gwas_beta_sq, gwas_beta_var, gwas_ld_scores, eqtl_beta_sq, eqtl_beta_var, eqtl_ld_scores,eqtl_mask, alpha_sq_variable, eqtl_beta_sq_variable, beta_sq_variable, n_snps, gwas_noise, eqtl_noise):
	...
	eqtl_log_like = compute_univariate_gaussian_log_like(eqtl_beta_sq[eqtl_indices], eqtl_beta_var[eqtl_indices] + big_eqtl_beta_sq_variable[eqtl_indices], tf.math.softplus(eqtl_noise))


	loss = -tf.reduce_sum(gwas_log_like) - tf.reduce_sum(eqtl_log_like)
	#loss = -tf.reduce_sum(gwas_log_like) 
...
def med_h2_with_sumstat_ldsc(gwas_beta, gwas_beta_se, gwas_ld_scores, eqtl_beta, eqtl_beta_se, eqtl_ld_scores, eqtl_n_cis_snps, max_epochs=40000, conv_thresh=1e-12, intercept_variables=False, all_intercepts=True):
	...
	alpha_sq_variable = tf.Variable(initial_value=0.000000001,trainable=True, name='alpha_sq', dtype=tf.float32)
	eqtl_beta_sq_variable = tf.Variable(initial_value=np.ones(n_genes)*0.0001,trainable=True, name='eqtl_beta_sq', dtype=tf.float32)

	gwas_noise = tf.Variable(initial_value=1.0,trainable=True, name='gwas_noise', dtype=tf.float32)
	eqtl_noise = tf.Variable(initial_value=1.0,trainable=True, name='eqtl_noise', dtype=tf.float32)
	...
	for epoch_iter in range(max_epochs):
		...
		with tf.GradientTape() as tape:
			loss_value = sumstat_proportional_joint_reml_loss(gwas_beta_sq, gwas_beta_var,gwas_ld_scores, eqtl_beta_sq, eqtl_beta_var, eqtl_ld_scores,eqtl_mask, alpha_sq_variable, eqtl_beta_sq_variable, beta_sq_variable, n_snps, gwas_noise, eqtl_noise)
...
output_file = trait_med_h2_inference_dir + simulation_name_string+ '_' + str(eqtl_sample_size) + '_joint_ldsc.txt'
...
est_med_h2, est_nm_h2, est_gene_cis_h2 = med_h2_with_sumstat_ldsc(gwas_beta, gwas_beta_se, gwas_variant_ld_scores, eqtl_beta_mat, eqtl_beta_se_mat, eqtl_ld_score_mat, eqtl_n_cis_snps, max_epochs=6000, intercept_variables=True, all_intercepts=True)
