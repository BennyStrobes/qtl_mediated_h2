...
class Bayesian_LMM_RSS_med_h2_inference(object):
	...
	def update_delta_vars(self, v0=0.0, s_sq=0.0, cc=1e-6):
		...
		for gene in self.genes:
			...
			self.delta_vars[gene] = tau_sq/vv
	...
	def update_gamma_delta_and_alpha_in_single_window(self, window_name):
		...
		window_eqtl_Q = np.load(self.window_info[window_name]['eqtl_Q_file'])
		...
		if order_bool == 0:
			...
			self.update_delta_in_single_window(window_name, window_Q,window_eqtl_Q)
		elif order_bool == 1:
			# Update delta in single window
			self.update_delta_in_single_window(window_name, window_Q, window_eqtl_Q)	
			# Update gamma and alpha in single window
	...
	def update_delta_in_single_window(self, window_name, window_Q, window_eqtl_Q):
		...
		for gene in np.random.permutation(window_genes):
			...
			gene_gwas_Q = window_Q[:, window_cis_indices]
			gene_eqtl_Q = window_eqtl_Q[:, window_cis_indices]
			...
			delta, window_gwas_beta_resid, eqtl_gene_beta_resid = self.update_delta_for_single_gene(gene_gwas_Q, gene_eqtl_Q, delta, window_gwas_beta_resid, eqtl_gene_beta_resid, self.alpha[gene], self.delta_vars[gene], self.eqtl_resid_vars[gene], window_gwas_resid_var)
	...
	def update_delta_for_single_gene(self, gene_gwas_Q, gene_eqtl_Q, delta, window_gwas_beta_resid, eqtl_gene_beta_resid, gene_alpha, gene_delta_var, gene_resid_var, gwas_resid_var):
		# N-snps in the the gene
		gene_K = len(delta)

		# Precompute snp posterior variances
		gene_gwas_Q_sum_sq = np.sum(np.square(gene_gwas_Q),axis=0)
		gene_eqtl_Q_sum_sq = np.sum(np.square(gene_gwas_Q),axis=0)
		snp_posterior_vars = 1.0/((gene_gwas_Q_sum_sq*np.square(gene_alpha)*self.N_gwas/gwas_resid_var) + (gene_eqtl_Q_sum_sq*self.N_eqtl/gene_resid_var) + (1.0/gene_delta_var))


		for kk in np.random.permutation(range(gene_K)):

			# Re include current effect
			window_gwas_beta_resid = window_gwas_beta_resid + (gene_gwas_Q[:, kk]*delta[kk]*gene_alpha)
			eqtl_gene_beta_resid = eqtl_gene_beta_resid + (gene_eqtl_Q[:, kk]*delta[kk])

			# Compute posterior distribution
			gwas_term = (self.N_gwas/gwas_resid_var)*gene_alpha*np.dot(window_gwas_beta_resid, gene_gwas_Q[:, kk])
			eqtl_term = (self.N_eqtl/gene_resid_var)*np.dot(eqtl_gene_beta_resid, gene_eqtl_Q[:, kk])
	...
	def initialize_variables(self):
		...
		for gene in self.genes:
			...
			gene_Q = np.load(self.window_info[gene_window]['eqtl_Q_file'])[:, gene_cis_snp_indices]
			gene_mod = bayesian_lmm_pca_ss_h2_single_region.Bayesian_LMM(gene_beta_pc, gene_Q, self.N_eqtl)
			gene_mod.fit(burn_in_iterations=250, total_iterations=500)
			aa.append(np.mean(gene_mod.sampled_gamma_vars)*np.sum(gene_cis_snp_indices))
