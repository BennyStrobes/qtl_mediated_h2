...
import bayesian_lmm_rss_med_h2_in_sample_ld
...
def extract_gwas_info_for_each_window(gwas_beta, gwas_rsids, quasi_ld_window_summary_file):
	...
	for line in f:
		...
		eqtl_q_mat_file = q_mat_file.split('geno_gwas')[0] + 'geno_eqtl_300' + q_mat_file.split('geno_gwas')[1]
		eqtl_q_mat = np.load(eqtl_q_mat_file)
		eqtl_w_mat_file = w_premult_mat_file.split('geno_gwas')[0] + 'geno_eqtl_300' + w_premult_mat_file.split('geno_gwas')[1]
		eqtl_w_mat = np.load(eqtl_w_mat_file)
		...
		window_info[window_name]['eqtl_Q_file'] = eqtl_q_mat_file
		window_info[window_name]['eqtl_W_file'] = eqtl_w_mat_file	
...
def load_in_eqtl_data(eqtl_sumstat_file, window_info, window_names):
	...
	for gene_name in gene_names:
		...
		q_mat_file= window_info[gene_window_name]['eqtl_Q_file']
		w_mat_file = window_info[gene_window_name]['eqtl_W_file']
		q_mat = np.load(q_mat_file)
...
if resid_var_bool == 'True':
	tmp_output_file = trait_med_h2_inference_dir + simulation_name_string + '_eqtl_' + str(N_eqtl) + '_resid_var_variable_rss_tmp_res_in_samp.txt'
	mod = bayesian_lmm_rss_med_h2_in_sample_ld.Bayesian_LMM_RSS_med_h2_inference(window_info, gene_info, N_gwas, N_eqtl, tmp_output_file)
