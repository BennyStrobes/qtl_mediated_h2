...
def load_in_eqtl_data3(eqtl_sumstat_file, snp_name_to_position, window_name_to_ld_files, eqtl_sample_size, num_snps, alt_simulated_learned_gene_models_dir, simulation_name_string, gwas_variant_ld_scores, eqtl_ld='out_of_sample', ldsc_variance=False, single_tissue=False):
	# First get list of gene names
	gene_names = []
	gene_classes = []
	f = open(eqtl_sumstat_file)
	...
	hat_sum_squared_eqtl_sumstats = np.zeros(num_snps)
	...
	for gg_iter, gene_name in enumerate(gene_names):
		...
		# Skip genes with negative ldsc variance if using ldsc variance
		if ldsc_variance and gene_ldsc_h2 <= 0.0:
			continue
		
		# QUick error check to make sure snps line up
		gene_snp_names = np.load(gene_snp_name_file)

		if np.array_equal(gene_rsids[gene_cis_snp_indices==1], gene_snp_names) == False:
			print('assumption erororo')
			pdb.set_trace()
		...
		hat_sum_squared_eqtl_sumstats[gene_snp_positions] = hat_sum_squared_eqtl_sumstats[gene_snp_positions] + np.square(eqtl_gene_beta) - np.square(eqtl_gene_beta_se)
	...
	eqtl_fourth_moment_scores = np.zeros(eqtl_ld_scores.shape)
	for kk in range(eqtl_fourth_moment_scores.shape[1]):
		eqtl_fourth_moment_scores[:, kk] = np.sum(np.transpose(eqtl_ld_scores)*eqtl_ld_scores[:,kk],axis=0)
	
	eqtl_second_moment_scores = np.sum(eqtl_ld_scores,axis=1)*gwas_variant_ld_scores


	return hat_sum_squared_eqtl_sumstats, eqtl_fourth_moment_scores, eqtl_second_moment_scores, n_genes, gene_cis_h2
...
###################################
'''
output_file = trait_med_h2_inference_dir + simulation_name_string + '_eqtl_SS_' + str(eqtl_sample_size) + '_mixed_ld4m_estimates.txt'
...
E_sum_squared_marginal_eqtls, eqtl_fourth_moment_scores, eqtl_second_moment_scores, n_genes, gene_cis_h2 = load_in_eqtl_data3(eqtl_sumstat_file, snp_name_to_position, window_to_ld_files ,eqtl_sample_size, len(gwas_rsids), alt_simulated_learned_gene_models_dir, simulation_name_string,gwas_variant_ld_scores, eqtl_ld='out_of_sample', ldsc_variance=False)



# Run Mixed ld4m
joint_ld4_scores = np.hstack((np.transpose(eqtl_second_moment_scores.reshape(1,-1)), eqtl_fourth_moment_scores))
mixed_4m_sumstats = (np.square(gwas_beta) - np.square(gwas_beta_se))*E_sum_squared_marginal_eqtls
model = sm.OLS(mixed_4m_sumstats, joint_ld4_scores)
results = model.fit()
# Extract estimated parameters
nm_h2 = results.params[0]*len(gwas_rsids)
per_tissue_h2 = results.params[1:]*n_genes*gene_cis_h2
t.write('mixed_ld4m\t' + str(sim_h2) + '\t' + str(sim_nm_h2) + '\t' + str(sim_med_h2) + '\t' + str(nm_h2 + np.sum(per_tissue_h2)) + '\t' + str(nm_h2) + '\t' + str(np.sum(per_tissue_h2)) + '\t' + ','.join(per_tissue_h2.astype(str)) + '\n')
t.close()
print(output_file)
'''

###################################
# 1 tissue (unpermuted eqtls)
###################################
output_file = trait_med_h2_inference_dir + simulation_name_string + '_eqtl_SS_' + str(eqtl_sample_size) + '_mixed_ld4m_estimates_single_tissue.txt'
t = open(output_file,'w')
t.write('method\tsim_tot_h2\tsim_nm_h2\tsim_med_h2\test_tot_h2\test_nm_h2\test_total_med_h2\test_per_tissue_med_h2\n')
...
###################################
'''
output_file = trait_med_h2_inference_dir + simulation_name_string + '_eqtl_SS_' + str(eqtl_sample_size) + '_mixed_ld4m_estimates_permuted_eqtls.txt'
t = open(output_file,'w')
t.write('method\tsim_tot_h2\tsim_nm_h2\tsim_med_h2\test_tot_h2\test_nm_h2\test_total_med_h2\test_per_tissue_med_h2\n')
...
E_sum_squared_marginal_eqtls, eqtl_fourth_moment_scores, eqtl_second_moment_scores, n_genes, gene_cis_h2 = load_in_eqtl_data3(eqtl_sumstat_file, snp_name_to_position, window_to_ld_files ,eqtl_sample_size, len(gwas_rsids), alt_simulated_learned_gene_models_dir, perm_simulation_name_string,gwas_variant_ld_scores, eqtl_ld='out_of_sample', ldsc_variance=False)
...
###################################
output_file = trait_med_h2_inference_dir + simulation_name_string + '_eqtl_SS_' + str(eqtl_sample_size) + '_mixed_ld4m_estimates_single_tissue_permuted_eqtls.txt'
t = open(output_file,'w')
t.write('method\tsim_tot_h2\tsim_nm_h2\tsim_med_h2\test_tot_h2\test_nm_h2\test_total_med_h2\test_per_tissue_med_h2\n')
