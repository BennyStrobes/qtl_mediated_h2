...
def sumstat_proportional_joint_reml_loss(gwas_beta, gwas_beta_var, gwas_ld_scores, eqtl_beta, eqtl_beta_var, eqtl_ld_scores,eqtl_mask, alpha_sq_variable, eqtl_beta_sq_variable, beta_sq_variable, n_snps, gene_to_class_index_matrix, per_snp_eqtl_h2_est_lb, per_snp_gwas_h2_est_lb, gwas_noise, eqtl_noise):
	...
	eqtl_log_like = compute_univariate_gaussian_log_like(eqtl_beta[eqtl_indices], 0.0, eqtl_beta_var[eqtl_indices] + eqtl_noise + big_eqtl_beta_sq_variable2[eqtl_indices])
...
def med_h2_with_sumstat_reml(gwas_beta, gwas_beta_se, gwas_ld_scores, eqtl_beta, eqtl_beta_se, eqtl_ld_scores, eqtl_n_cis_snps, eqtl_classes, max_epochs=40000, conv_thresh=1e-12, intercept_variables=False, learning_rate=5e-6):
	...
	for epoch_iter in range(max_epochs):
		...
		with tf.GradientTape() as tape:
			loss_value = sumstat_proportional_joint_reml_loss(gwas_beta, gwas_beta_var,gwas_ld_scores, eqtl_beta, eqtl_beta_var,eqtl_ld_scores,eqtl_mask, alpha_sq_variable, eqtl_beta_sq_variable, beta_sq_variable, n_snps, gene_to_class_index_matrix, per_snp_eqtl_h2_est_lb, per_snp_gwas_h2_est_lb, gwas_noise, eqtl_noise)
...
output_file = trait_med_h2_inference_dir + simulation_name_string+ '_' + str(eqtl_sample_size) + '_joint_reml_w_intercepts.txt'
t = open(output_file,'w')
t.write('eQTL_SS\tsim_h2\tsim_med_h2\tsim_nm_h2\test_med_h2_joint_reml\test_med_h2_per_tissue_joint_reml\test_nm_h2_joint_reml\test_mean_eqtl_h2_joint_reml\test_h2_ldsc\test_h2_ldsc_constrained_intercept\n')
...
est_total_med_h2, est_per_tissue_med_h2, est_nm_h2, est_gene_cis_h2 = med_h2_with_sumstat_reml(gwas_beta, gwas_beta_se, gwas_variant_ld_scores, eqtl_beta_mat, eqtl_beta_se_mat, eqtl_ld_score_mat, eqtl_n_cis_snps, eqtl_classes, max_epochs=50000, intercept_variables=True, learning_rate=1e-6)


t.write(str(eqtl_sample_size) + '\t' + str(sim_h2) + '\t' + str(sim_med_h2) + '\t' + str(sim_nm_h2) + '\t' + str(est_total_med_h2) + '\t' + ','.join(est_per_tissue_med_h2.astype(str)) + '\t' + str(est_nm_h2) + '\t' + str(np.mean(est_gene_cis_h2)) + '\t' + str(ldsc_snp_h2) + '\t' + str(ldsc_constrained_intercept_snp_h2) + '\n')
