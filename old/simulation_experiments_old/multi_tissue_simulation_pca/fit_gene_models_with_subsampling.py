...
def simulate_gene_expression_and_fit_gene_model_for_all_genes_shell(simulated_causal_eqtl_effect_summary_file, eqtl_sample_size, simulation_name_string, processed_genotype_data_dir, simulated_learned_gene_models_dir, chrom_num, n_subsamples=10, version = 'big'):
	...
	# Sumstats
	if version == 'big':
		sumstat_output_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + str(eqtl_sample_size) + '_big_window_eqtl_sumstats_with_subsampling.txt'
		gene_model_output_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + str(eqtl_sample_size) + '_big_window_gene_model_info_with_subsampling.txt'
	else:
		sumstat_output_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + str(eqtl_sample_size) + '_small_window_eqtl_sumstats_with_subsampling.txt'
		gene_model_output_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + str(eqtl_sample_size) + '_small_window_gene_model_info_with_subsampling.txt'
	
	subsample_groups = []
	for subsample_iter in range(n_subsamples):
		subsample_groups.append(np.array_split(np.random.permutation(np.arange(eqtl_sample_size)),2))


	t = open(sumstat_output_file,'w')
	t.write('gene_id\tgene_class\tvariant_id\twindow_name\tcis_snp\teffect_size\teffect_size_se')
	# Add subsamples
	for subsample_iter in range(n_subsamples):
		t.write('\t' + 'effect_size_subsample_' + str(subsample_iter) + '_a' + '\t' + 'effect_size_se_subsample_' + str(subsample_iter) + '_a' + '\t' + 'effect_size_subsample_' + str(subsample_iter) + '_b' + '\t' + 'effect_size_se_subsample_' + str(subsample_iter) + '_b')
	t.write('\n')
	...
	for line in f:
		...
		for tiss_iter in range(n_tiss):
			...
			sim_expr = simulate_gene_expression(gene_geno, tissue_sim_causal_eqtl_effect_sizes)
			# Standardize simulated gene expression
			sim_stand_expr = (sim_expr - np.mean(sim_expr))/np.std(sim_expr)


			# Save expression and genotype to output file 
			expr_output_file = simulated_learned_gene_models_dir + simulation_name_string + '_' + str(eqtl_sample_size) + '_' + version + '_' + ensamble_id + ':tissue' + str(tiss_iter) + '_expression.npy' 
			...
			subsampled_marginal_effects_a = []
			subsampled_marginal_effects_se_a = []
			subsampled_marginal_effects_b = []
			subsampled_marginal_effects_se_b = []

			for subsample_iter in range(n_subsamples):
				group1_indices, group2_indices = subsample_groups[subsample_iter]

				# Group 1
				# SHOULD CONSIDER NOT STANDARDIZING!!!!
				tmp_expr = (sim_stand_expr[group1_indices] - np.mean(sim_stand_expr[group1_indices]))/np.std(sim_stand_expr[group1_indices])
				tmp_geno = standardize_genotype_dosage_matrix(gene_geno_big[group1_indices,:])
				subsampled_1_marginal_effects, subsampled_1_marginal_effects_se = compute_marginal_regression_coefficients(tmp_expr, tmp_geno)

				subsampled_marginal_effects_a.append(subsampled_1_marginal_effects)
				subsampled_marginal_effects_se_a.append(subsampled_1_marginal_effects_se)

				# Group 2
				tmp_expr = (sim_stand_expr[group2_indices] - np.mean(sim_stand_expr[group2_indices]))/np.std(sim_stand_expr[group2_indices])
				tmp_geno = standardize_genotype_dosage_matrix(gene_geno_big[group2_indices,:])
				subsampled_2_marginal_effects, subsampled_2_marginal_effects_se = compute_marginal_regression_coefficients(tmp_expr, tmp_geno)				
				subsampled_marginal_effects_b.append(subsampled_2_marginal_effects)
				subsampled_marginal_effects_se_b.append(subsampled_2_marginal_effects_se)


			# Print eqtl sumstats to output file
			for snp_iter, snp_rsid in enumerate(cis_rsids_big):
				t.write(ensamble_id+ ':tissue' + str(tiss_iter) + '\t' + 'tissue' + str(tiss_iter) + '\t' + snp_rsid + '\t' + window_name + '\t' + str((big_window_cis_subset_indices*1.0)[snp_iter]) + '\t' + str(marginal_effects[snp_iter]) + '\t' + str(marginal_effects_se[snp_iter]))
				for subsample_iter in range(n_subsamples):
					t.write('\t' + str(subsampled_marginal_effects_a[subsample_iter][snp_iter]))
					t.write('\t' + str(subsampled_marginal_effects_se_a[subsample_iter][snp_iter]))
					t.write('\t' + str(subsampled_marginal_effects_b[subsample_iter][snp_iter]))
					t.write('\t' + str(subsampled_marginal_effects_se_b[subsample_iter][snp_iter]))
				t.write('\n')
	f.close()
	t.close()
	print(sumstat_output_file)
	return
