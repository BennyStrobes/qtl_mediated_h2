...
n_eqtl_datasets = int(sys.argv[8])
n_causal_eqtl_datasets = int(sys.argv[9])
...
n_causal_genes_per_causal_eqtl_dataset = int(mediated_h2/(n_causal_eqtl_datasets*per_element_heritability))
n_causal_snps = int(nm_h2/per_element_heritability)


# Simulated causal non-mediated variant to trait effect sizes
non_mediated_variant_causal_effect_sizes_file = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_nm_variant_to_trait_effect_sizes.txt'
...
# Initialize vector of mediated variant effects (aggregrateed across eqtl data sets)
mediated_variant_effect_sizes = np.zeros(n_snps)

# LOop through eqtl data sets
for eqtl_dataset_iter in range(n_causal_eqtl_datasets):
	# Get number of genes in this causal eqtl dataset
	simulated_causal_eqtl_effect_summary_file = simulated_eqtl_data_dir + simulation_name_string + 'causal_eqtl_effect_summary_eqtl_dataset_' + str(eqtl_dataset_iter + 1) + '.txt'
	...
	gene_causal_effects[np.random.choice(np.arange(n_genes), size=n_causal_genes_per_causal_eqtl_dataset, replace=False)] = np.random.normal(loc=0.0, scale=np.sqrt(per_element_heritability/.05),size=n_causal_genes_per_causal_eqtl_dataset)


	# Print gene causal effects to output file
	mediated_gene_causal_effect_sizes_file = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_mediated_gene_to_trait_effect_sizes_eqtl_dataset_' + str(eqtl_dataset_iter+1) + '.txt'
...
total_genetic_var_output = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_simulated_genetic_var.npy'
np.save(total_genetic_var_output, total_genetic_var)

total_nm_genetic_var_output = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_simulated_nm_genetic_var.npy'
np.save(total_nm_genetic_var_output, total_nm_genetic_var)

total_mediated_genetic_var_output = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_simulated_mediated_genetic_var.npy'
...
# Save to output
trait_values_output = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_simulated_trait.txt'
