...
#SBATCH -t 0-14:40                         # Runtime in D-HH:MM format
...
simulation_name_string="simulation_"${simulation_number}"_"${sub_simulation_number}"_n_genes_"${n_genes}"_frac_genes_h2_"${fraction_genes_cis_h2}"_ele_h2_"${per_element_heritability}"_tot_h2_"${total_heritability}"_frac_mediated_"${fraction_expression_mediated_heritability}"_ge_h2_"${ge_h2}"_n_eqtl_data_"${n_eqtl_datasets}"_"${n_causal_eqtl_datasets}"_"
echo $simulation_name_string
##########################
# Part 1: Simulate variant-gene links and causal eqtl effects
###########################
####**
python3 simulate_variant_gene_links_and_causal_eqtl_effects_across_multiple_eqtl_datasets.py $simulation_name_string $processed_genotype_data_dir $simulated_eqtl_data_dir $n_genes $fraction_genes_cis_h2 $ge_h2 $n_eqtl_datasets
...
do
	for eqtl_dataset_iter in $(seq 1 $n_eqtl_datasets); do 
		python3 simulate_gene_expression.py $simulation_name_string $processed_genotype_data_dir $simulated_eqtl_data_dir $simulated_expression_data_dir $eqtl_ss $eqtl_dataset_iter
	done
done
...
do
	for eqtl_dataset_iter in $(seq 1 $n_eqtl_datasets); do 
		python3 learn_gene_models.py $simulation_name_string $processed_genotype_data_dir $simulated_eqtl_data_dir $simulated_expression_data_dir $simulated_gene_models_dir $eqtl_ss $ge_h2 $eqtl_dataset_iter
	done
done
...
####**
# NEUTRAL SELECTION
python3 simulate_gwas_data_neutral_selection.py $simulation_name_string $processed_genotype_data_dir $simulated_eqtl_data_dir $simulated_gwas_data_dir $total_heritability $fraction_expression_mediated_heritability $per_element_heritability $n_eqtl_datasets $n_causal_eqtl_datasets
...
for eqtl_ss in "${eqtl_sample_size_arr[@]}"
do
	python3 perform_med_h2_ldsc_inference.py $simulation_name_string $simulated_gwas_data_dir $mediated_h2_results_dir $processed_genotype_data_dir $simulated_eqtl_data_dir $selection $eqtl_ss $simulated_gene_models_dir $n_eqtl_datasets
done
