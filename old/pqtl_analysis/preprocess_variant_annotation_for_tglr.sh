...
ldsc_code_dir="$1"
orig_hapmap3_rsid_file="$2"
baselineLD_anno_dir="$3"
ref_1kg_genotype_dir="$4"
tglr_variant_annotations_dir="$5"


hapmap3_rsid_file=$tglr_variant_annotations_dir"hm3_rsid_list.txt"
python3 convert_hm3_rsid_file.py $orig_hapmap3_rsid_file $hapmap3_rsid_file
...
for chrom_num in $(seq 1 $(($num_chrom))); do 
	...
	python ${ldsc_code_dir}ldsc.py\
		--l2\
		--bfile ${ref_1kg_genotype_dir}1000G.EUR.QC.${chrom_num}\
		--ld-wind-cm 1\
	...
	plink2 --bfile ${ref_1kg_genotype_dir}"1000G.EUR.QC."${chrom_num} --extract ${hapmap3_rsid_file} --threads 1 --make-bed --keep-allele-order --out ${tglr_variant_annotations_dir}"_100G_regression_snps_only."${chrom_num}
