...
expression_score_annotation_stem=${tglr_expression_score_dir}"tglr_expression_scores_"${method}"."
if false; then
python3 organize_tglr_expression_scores_across_tissues.py $ldsc_genotype_intercept_annotation_stem $tglr_expression_score_dir $expression_score_annotation_stem $method
fi
...
sed 1d $non_redundent_summary_statistics_file | while read gwas_trait_name h2 h2_se h2_z intercept; do
	...
	tglr_output_stem=${tglr_results_dir}${gwas_trait_name}"_"${method}"_baselineLD_no_qtl_tglr"
	...
	tglr_output_stem=${tglr_results_dir}${gwas_trait_name}"_"${method}"_genotype_intercept_tglr"
	module load python/2.7.12
