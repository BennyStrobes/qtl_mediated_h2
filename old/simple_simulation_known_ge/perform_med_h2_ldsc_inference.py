...
def mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq(gwas_z_scores, N_gwas, LD, causal_eqtl_effects, causal_eqtl_indices, noise_ratios):
	# Get number of genes
	n_genes = len(causal_eqtl_indices)
	# Get number of snps
	n_snps = len(gwas_z_scores)
...
nm_sldsc_standardize_true_gene, med_sldsc_standardize_true_gene = sldsc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices, np.zeros(n_genes))

nm_sldsc_raw_true_gene, med_sldsc_raw_true_gene = mesc_with_variants_and_noisy_genetic_gene_expression_no_twas_chi_sq(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices, np.zeros(n_genes))
...
#nm_he_sldsc_standardize_true_gene, med_he_sldsc_standardize_true_gene = sldsc_he_with_variants_and_genetic_gene_expression_no_twas_chi_sq(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices,LD_LD_t)

#nm_he_sldsc_raw_true_gene, med_he_sldsc_raw_true_gene = mesc_he_with_variants_and_genetic_gene_expression_no_twas_chi_sq(gwas_z_scores, N_gwas, LD, true_causal_eqtl_effects, causal_eqtl_indices,LD_LD_t)


# Now consider effect of adding noise 




# Print results to output
output_file = mediated_h2_results_dir + simulation_name_string+ selection + '_selection_'+ 'med_h2_ldsc_style_summary.txt'
t = open(output_file,'w')
# Header
t.write('sim_h2\tsim_nm_h2\tsim_med_h2\t')
t.write('nm_sldsc_standardize_true_gene\tmed_sldsc_standardize_true_gene\t')
...
t.write(str(nm_sldsc_standardize_true_gene) + '\t' + str(med_sldsc_standardize_true_gene) + '\t')
t.write(str(nm_sldsc_raw_true_gene) + '\t' + str(med_sldsc_raw_true_gene) + '\n')
#t.write(str(nm_he_sldsc_standardize_true_gene) + '\t' + str(med_he_sldsc_standardize_true_gene) + '\t')
#t.write(str(nm_he_sldsc_raw_true_gene) + '\t' + str(med_he_sldsc_raw_true_gene) + '\n')
