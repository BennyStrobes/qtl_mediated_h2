...
gene_causal_effects[np.random.choice(np.arange(50), size=n_causal_genes, replace=False)] = np.random.normal(loc=0.0, scale=np.sqrt(per_element_heritability/.05),size=n_causal_genes)

# Print gene causal effects to output file
mediated_gene_causal_effect_sizes_file = simulated_gwas_data_dir + simulation_name_string + 'neutral_selection_mediated_gene_to_trait_effect_sizes.txt'
