...
output_file = output_dir + 'single_gene_sparse_eqtl_unshared_causal_snps_extra_nm_var_in_gene_' + str(n_causal_eqtl_snps_string) + '_eqtl_snps.txt'
...
for n_causal_eqtl_snps in n_causal_eqtl_snps_arr:
	...
	gecs2_alpha_sq_est = []
	gecs2_alpha_sq_prime_est = []
	gecs3_alpha_sq_est = []
	gecs3_alpha_sq_prime_est = []
	gecs4_alpha_sq_est = []
	gecs4_alpha_sq_prime_est = []
	...
	for sim_iter in range(n_sims):
		...
		Y = beta_beta_transpose[upper_triangle_indices]
		cross_ld_scores = LD_t_LD[upper_triangle_indices]
		cross_delta_terms = delta_delta_transpose[upper_triangle_indices]
		...
		cross_delta_prime_terms = delta_prime_delta_prime_transpose[upper_triangle_indices]
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_prime_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs_alpha_sq_prime_est.append(gecs_model.params[1])


		# low_ld_t_ld_indices_10
		# Version of GE co-score regression
		beta_beta_transpose = np.dot(beta_marginal.reshape(-1,1), beta_marginal.reshape(1,-1))
		delta_delta_transpose = np.dot(delta_marginal.reshape(-1,1), delta_marginal.reshape(1,-1))
		Y = beta_beta_transpose[upper_triangle_indices][low_ld_t_ld_indices_10]
		cross_ld_scores = LD_t_LD[upper_triangle_indices][low_ld_t_ld_indices_10]
		cross_delta_terms = delta_delta_transpose[upper_triangle_indices][low_ld_t_ld_indices_10]
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs2_alpha_sq_est.append(gecs_model.params[1])

		# Alt tissue
		delta_prime_delta_prime_transpose = np.dot(delta_prime_marginal.reshape(-1,1), delta_prime_marginal.reshape(1,-1))
		cross_delta_prime_terms = delta_prime_delta_prime_transpose[upper_triangle_indices][low_ld_t_ld_indices_10]
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_prime_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs2_alpha_sq_prime_est.append(gecs_model.params[1])
		...
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs3_alpha_sq_est.append(gecs_model.params[1])
		...
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_prime_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs3_alpha_sq_prime_est.append(gecs_model.params[1])



		# low_ld_t_ld_indices_half
		# Version of GE co-score regression
		beta_beta_transpose = np.dot(beta_marginal.reshape(-1,1), beta_marginal.reshape(1,-1))
		delta_delta_transpose = np.dot(delta_marginal.reshape(-1,1), delta_marginal.reshape(1,-1))
		Y = beta_beta_transpose[upper_triangle_indices][low_ld_t_ld_indices_half]
		cross_ld_scores = LD_t_LD[upper_triangle_indices][low_ld_t_ld_indices_half]
		cross_delta_terms = delta_delta_transpose[upper_triangle_indices][low_ld_t_ld_indices_half]
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs4_alpha_sq_est.append(gecs_model.params[1])

		# Alt tissue
		delta_prime_delta_prime_transpose = np.dot(delta_prime_marginal.reshape(-1,1), delta_prime_marginal.reshape(1,-1))
		cross_delta_prime_terms = delta_prime_delta_prime_transpose[upper_triangle_indices][low_ld_t_ld_indices_half]
		X = np.transpose(np.vstack((cross_ld_scores, cross_delta_prime_terms)))
		gecs_model = sm.OLS(Y,X).fit()
		gecs4_alpha_sq_prime_est.append(gecs_model.params[1])
	...
	t = formal_print_95_ci(t, gecs_alpha_sq_est, 'gecs_all_snp_pairs', str(n_causal_eqtl_snps))
	gecs_alpha_sq_prime_est = np.asarray(gecs_alpha_sq_prime_est)
	t = formal_print_95_ci(t, gecs_alpha_sq_prime_est, 'gecs_prime_all_snp_pairs', str(n_causal_eqtl_snps))

	gecs2_alpha_sq_est = np.asarray(gecs2_alpha_sq_est)
	t = formal_print_95_ci(t, gecs2_alpha_sq_est, 'gecs_10_thresh', str(n_causal_eqtl_snps))
	gecs2_alpha_sq_prime_est = np.asarray(gecs2_alpha_sq_prime_est)
	t = formal_print_95_ci(t, gecs2_alpha_sq_prime_est, 'gecs_prime_10_thresh', str(n_causal_eqtl_snps))


	gecs3_alpha_sq_est = np.asarray(gecs3_alpha_sq_est)
	t = formal_print_95_ci(t, gecs3_alpha_sq_est, 'gecs_2_thresh', str(n_causal_eqtl_snps))
	gecs3_alpha_sq_prime_est = np.asarray(gecs3_alpha_sq_prime_est)
	t = formal_print_95_ci(t, gecs3_alpha_sq_prime_est, 'gecs_prime_2_thresh', str(n_causal_eqtl_snps))

	gecs4_alpha_sq_est = np.asarray(gecs4_alpha_sq_est)
	t = formal_print_95_ci(t, gecs4_alpha_sq_est, 'gecs_.5_thres_thresh', str(n_causal_eqtl_snps))
	gecs4_alpha_sq_prime_est = np.asarray(gecs4_alpha_sq_prime_est)
	t = formal_print_95_ci(t, gecs4_alpha_sq_prime_est, 'gecs_prime_.5_thresh', str(n_causal_eqtl_snps))
